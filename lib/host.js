// ============================================================
// dsh-chime-alerts · 宿主半（Host half）— v0.3.20
...
return {
  ...
  apply(ctx) {
    ...
    async function resolveUserHome() {
      ...
      try {
        ...
        const handle = sub.spawn({
          argv: ['C:\\Windows\\System32\\cmd.exe', '/d', '/c', 'echo %USERPROFILE%'],
          cwd: 'C:\\',
        ...
        if (res.exitCode !== 0 || !/^[a-zA-Z]:[\\/]/u.test(text)) return null
        userHomeCache = text
        return text
    ...
    async function resolveStorageRoot() {
      ...
      try {
        if (baseCwd !== null && SYS_ROOT_RE.test(baseCwd)) {
          const home = await resolveUserHome()
          if (home !== null) baseCwd = home + '\\.dsh\\plugins\\dsh-chime-alerts'
        }
      } catch (err) {}
      return baseCwd
    }
    ...
    async function migrateLegacy() {
      ...
      try {
        ...
        if (!SYS_ROOT_RE.test(legacyRoot)) return
    ...
    handle('sysget', async () => {
      ...
      if (data !== null && typeof data === 'object') {
        ...
        if (data.hostSounds !== null && typeof data.hostSounds === 'object') {
          for (const k of Object.keys(DEFAULT_WAV)) {
            if (typeof data.hostSounds[k] === 'string' && data.hostSounds[k] !== '') hs[k] = data.hostSounds[k]
          }
        }
      }
      hostSounds = hs
      return { ok: fsSvc !== null, hostBeep, hostSounds: hs, dir: dirPath }
    })
    ...
    handle('sysset', async (args) => {
      ...
      if (hostBeep === null && hsIn === null) return { ok: false, reason: 'bad-args' }
      ...
      const out = await writeJson(SETTINGS_FILE, { hostBeep: alwaysBeep, hostSounds })
    ...
    handle('sysbeep', (args) => {
      ...
      if (DEFAULT_WAV[kind] === undefined) return { ok: false, reason: 'unknown-kind' }
