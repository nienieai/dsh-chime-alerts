  ...
  factory: (require) => {
    ...
    function hostFetch(path, opts) {
      try {
        ...
        return fetch(HOST_API_BASE + path, Object.assign({ headers: { 'content-type': 'application/json' } }, opts || {}))
    ...
    function useVersion() {
      ...
      React.useEffect(function () {
        var fn = function () { pair[1](pair[0] + 1) }
      ...
      return React.createElement('div', { className: 'snd-page' },
        ...
        React.createElement('div', { className: 'snd-master' },
          React.createElement('div', null,
            ...
            React.createElement('span', { className: 'snd-desc' }, T('事件时宿主播放系统提示音（Windows 系统 wav / Linux 主题音 / macOS 系统音）', 'Plays the OS system sound on events (Windows wav / Linux theme sound / macOS system sound)'))),
        ...
        React.createElement('div', { className: 'snd-note' },
          T('静态版提示：事件检测基于会话快照，「插件授权」由宿主蜂鸣兜底，「其他打断」并入「任务完成」；宿主蜂鸣设置保存到宿主磁盘（%USERPROFILE%\\.dsh\\plugins\\dsh-chime-alerts\\dsh-chime-alerts-settings.json），自定义音频存浏览器（≤3MB）。', 'Static build notes: detection uses session snapshots; plugin approval falls back to host beep and interrupts merge into completion; host beep is stored on host disk (settings.json under .dsh/plugins/dsh-chime-alerts); custom audio is stored in the browser (≤3MB).')))
    }
