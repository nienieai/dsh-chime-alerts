    ...
    function SettingsPage() {
      ...
      return React.createElement('div', { className: 'snd-page' },
        ...
        React.createElement('div', { className: 'snd-note' },
          T('静态版提示：事件检测基于会话快照，「插件授权」与「其他打断」无法从快照识别——前者由宿主蜂鸣（系统音，需在宿主配置开启）兜底，后者并入「任务完成」；自定义音频存浏览器（≤3MB）。', 'Static build notes: detection uses session snapshots; plugin approval is covered by host beep only and interrupts merge into completion; custom audio is stored in the browser (≤3MB).')))
    }
