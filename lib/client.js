// ============================================================
// dsh-chime-alerts · 客户端半（Client half）— v0.3.17
...
return {
  ...
  apply(ctx) {
    ...
    // v0.3.17：九种音各有 1~4 个音符、跳跃音型（非连续音阶），且每个音符的时值/间隔各不相同（节奏化）；
    // 全部 sine 基音 + sine 2 倍频泛音。语义与节奏设计：
    //   complete    4声 短-短-短-长 上行琶音（叮叮叮—咚，胜利收尾）
    //   subcomplete 1声 轻叮（子任务轻提示）
    //   jobdone     3声 短-短-长 跳进上行（完成后一声长叹）
    //   approval    2声 慢"叮—咚"高→低（门铃单遍，请人来处理）
    //   question    2声 上扬双音、尾音长（疑问句末尾上扬）
    //   planreview  3声 紧凑下行（计划评审落定）
    //   goalblocked 3声 重复低音+再下行（卡住/受阻）
    //   interrupt   2声 长音后突停短音（"叮—噗"被打断）
    //   jobfail     3声 快速坠落+长尾音（失败叹息）
    // 尾音 0.12–0.8s 不等，保留 0.25s 余韵铃感。
    const PATTERNS = {
      complete: [[523.25, 0, 0.12, 'sine'], [659.25, 0.12, 0.12, 'sine'], [783.99, 0.24, 0.12, 'sine'], [1046.5, 0.36, 0.8, 'sine']],
      subcomplete: [[783.99, 0, 0.35, 'sine']],
      jobdone: [[392, 0, 0.1, 'sine'], [523.25, 0.1, 0.1, 'sine'], [783.99, 0.2, 0.6, 'sine']],
      approval: [[880, 0, 0.25, 'sine'], [659.25, 0.3, 0.7, 'sine']],
      question: [[523.25, 0, 0.15, 'sine'], [783.99, 0.2, 0.5, 'sine']],
      planreview: [[987.77, 0, 0.12, 'sine'], [783.99, 0.12, 0.12, 'sine'], [523.25, 0.24, 0.55, 'sine']],
      goalblocked: [[349.23, 0, 0.18, 'sine'], [349.23, 0.18, 0.18, 'sine'], [261.63, 0.4, 0.7, 'sine']],
      interrupt: [[880, 0, 0.25, 'sine'], [440, 0.27, 0.12, 'sine']],
      jobfail: [[1046.5, 0, 0.09, 'sine'], [698.46, 0.09, 0.09, 'sine'], [493.88, 0.18, 0.6, 'sine']]
    }
    ...
    const SOUND_OPTIONS = [
      ...
      { value: 'complete', label: T('完成音 · 短短短长琶音', 'Complete · quick quick quick long') },
