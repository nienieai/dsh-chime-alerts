// ============================================================
// dsh-chime-alerts · 客户端半（Client half）— v0.3.18
...
return {
  ...
  apply(ctx) {
    ...
    // v0.3.18：九种音全面灵动——1~4 个音符、连续/跳跃音阶、相同/不同间隔、相同/不同时长、
    // 每音音量因子(渐强/渐弱)、音色变化(sine/triangle)、尾音长短;事件整体音量因子(后台任务低调)。
    // 每个音符 = [频率, 起始秒, 时长秒, 波形, 音量因子];事件对象 = { vol: 整体音量, notes: [...] }。
    // 语义与节奏设计：
    //   complete    4声 短-短-短-长 上行琶音、渐强、尾音 0.85s(叮叮叮—咚,胜利收尾)
    //   subcomplete 1声 轻叮(子任务轻提示)
    //   jobdone     3声 短短长 跳进上行、渐弱、整体低调(vol 0.5,后台不打扰)
    //   approval    2声 慢"叮—咚"高→低、渐强(门铃单遍,请人来处理)
    //   question    2声 上扬双音、渐强(疑问句末尾上扬)
    //   planreview  3声 紧凑下行、渐弱(评审落定)
    //   goalblocked 3声 重复低音+下行、渐强(卡住/受阻)
    //   interrupt   2声 长音后突停(被打断)
    //   jobfail     3声 快速坠落+长叹、渐弱、整体低调(vol 0.5,后台不打扰)
    const PATTERNS = {
      complete: { notes: [[523.25, 0, 0.1, 'sine', 0.6], [659.25, 0.12, 0.1, 'sine', 0.75], [783.99, 0.26, 0.12, 'triangle', 0.9], [1046.5, 0.42, 0.85, 'sine', 1]] },
      subcomplete: { notes: [[783.99, 0, 0.4, 'sine', 1]] },
    ...
    async function playBrowser(kind, cfg) {
      ...
      // v0.3.15：音量提升；v0.3.18：再加大（基音 0.45→0.55），每音独立音量因子（渐强/渐弱），
      // 事件整体音量因子（后台任务低调），音色每音可不同（sine/triangle）。
      for (const item of pattern) {
        ...
        gain.gain.exponentialRampToValueAtTime(0.55 * v * evVol * nv, t + 0.012)
    ...
    const SOUND_OPTIONS = [
      ...
      { value: 'complete', label: T('完成音 · 渐强琶音', 'Complete · crescendo arpeggio') },
      { value: 'subcomplete', label: T('子任务音 · 单声轻叮', 'Subtask · single soft ding') },
      { value: 'approval', label: T('授权音 · 慢叮咚', 'Approval · slow doorbell') },
      { value: 'question', label: T('提问音 · 上扬双音', 'Question · rising pair') },
      { value: 'planreview', label: T('评审音 · 渐弱下行', 'Review · descending fade') },
      { value: 'goalblocked', label: T('受阻音 · 卡住低音', 'Blocked · stuck low tones') },
      { value: 'interrupt', label: T('打断音 · 突停', 'Interrupt · abrupt stop') },
      { value: 'jobfail', label: T('失败音 · 低调坠落', 'Failure · quiet fall') },
      { value: 'jobdone', label: T('后台完成音 · 低调轻响', 'Job done · quiet chime') },
