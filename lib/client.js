// ============================================================
// dsh-chime-alerts · 客户端半（Client half）— v0.3.15
...
return {
  ...
  apply(ctx) {
    ...
    // v0.3.15：九种音统一为四音符（更饱满），全部 sine 基音 + sine 2 倍频泛音；
    // 音型语法：上行琶音=完成/积极（complete / subcomplete / jobdone），
    // 叮咚×2 门铃=需要授权，四连上行=提问，四连下行=评审结束，
    // 低音慢四连下行=目标受阻，快四连下行=打断/失败。
    // 每音 0.1–0.2s 间隔、尾音 0.4–0.7s 长衰减（保留 0.25s 余韵铃感）。
    const PATTERNS = {
      complete: [[523.25, 0, 0.18, 'sine'], [659.25, 0.16, 0.18, 'sine'], [783.99, 0.32, 0.18, 'sine'], [1046.5, 0.48, 0.7, 'sine']],
      subcomplete: [[659.25, 0, 0.12, 'sine'], [783.99, 0.12, 0.12, 'sine'], [987.77, 0.24, 0.12, 'sine'], [1174.66, 0.36, 0.5, 'sine']],
      jobdone: [[587.33, 0, 0.14, 'sine'], [698.46, 0.14, 0.14, 'sine'], [880, 0.28, 0.14, 'sine'], [1174.66, 0.42, 0.55, 'sine']],
      approval: [[987.77, 0, 0.16, 'sine'], [739.99, 0.18, 0.16, 'sine'], [987.77, 0.4, 0.16, 'sine'], [739.99, 0.58, 0.6, 'sine']],
      question: [[523.25, 0, 0.12, 'sine'], [659.25, 0.12, 0.12, 'sine'], [783.99, 0.24, 0.12, 'sine'], [987.77, 0.36, 0.5, 'sine']],
      planreview: [[987.77, 0, 0.12, 'sine'], [783.99, 0.12, 0.12, 'sine'], [659.25, 0.24, 0.12, 'sine'], [523.25, 0.36, 0.55, 'sine']],
      goalblocked: [[392, 0, 0.2, 'sine'], [349.23, 0.2, 0.2, 'sine'], [311.13, 0.4, 0.2, 'sine'], [261.63, 0.6, 0.65, 'sine']],
      interrupt: [[523.25, 0, 0.1, 'sine'], [493.88, 0.1, 0.1, 'sine'], [440, 0.2, 0.1, 'sine'], [392, 0.3, 0.4, 'sine']],
      jobfail: [[493.88, 0, 0.1, 'sine'], [440, 0.1, 0.1, 'sine'], [392, 0.2, 0.1, 'sine'], [349.23, 0.3, 0.45, 'sine']]
    }
    ...
    const SOUND_OPTIONS = [
      ...
      { value: 'subcomplete', label: T('子任务音 · 四连上行', 'Subtask · four ascending notes') },
      { value: 'approval', label: T('授权音 · 叮咚门铃×2', 'Approval · two-tone bell ×2') },
      { value: 'question', label: T('提问音 · 四连上行', 'Question · four ascending notes') },
      { value: 'planreview', label: T('评审音 · 四连下行', 'Review · four descending notes') },
      { value: 'goalblocked', label: T('受阻音 · 低音四连', 'Blocked · low descending quad') },
      { value: 'interrupt', label: T('打断音 · 快四连下行', 'Interrupt · quick descending quad') },
      { value: 'jobfail', label: T('失败音 · 快四连下行', 'Failure · quick descending quad') },
      { value: 'jobdone', label: T('后台完成音 · 上行琶音', 'Job done · ascending arpeggio') },
