// ============================================================
// dsh-chime-alerts · 客户端半（Client half）— v0.3.14
...
return {
  ...
  apply(ctx) {
    ...
    // v0.3.14：九种音统一风格——全部 sine 基音 + sine 2 倍频泛音；
    // 音型语法：上行=完成/积极（complete 两音 / subcomplete 单音 / jobdone 两音），
    // 高→低双音门铃=需要授权，三连上行=提问，三连下行=评审结束，
    // 低音慢三连下行=目标受阻，快两音下行=打断，快三连下行=失败。
    const PATTERNS = {
      complete: [[659.25, 0, 0.25, 'sine'], [987.77, 0.2, 0.55, 'sine']],
      subcomplete: [[880, 0, 0.4, 'sine']],
      jobdone: [[587.33, 0, 0.15, 'sine'], [880, 0.16, 0.35, 'sine']],
      approval: [[987.77, 0, 0.2, 'sine'], [739.99, 0.24, 0.5, 'sine']],
      question: [[523.25, 0, 0.14, 'sine'], [659.25, 0.12, 0.14, 'sine'], [783.99, 0.24, 0.4, 'sine']],
      planreview: [[783.99, 0, 0.14, 'sine'], [659.25, 0.12, 0.14, 'sine'], [523.25, 0.24, 0.45, 'sine']],
      goalblocked: [[392, 0, 0.2, 'sine'], [349.23, 0.22, 0.2, 'sine'], [261.63, 0.44, 0.5, 'sine']],
      interrupt: [[523.25, 0, 0.13, 'sine'], [392, 0.14, 0.3, 'sine']],
      jobfail: [[494, 0, 0.12, 'sine'], [440, 0.13, 0.12, 'sine'], [392, 0.26, 0.35, 'sine']]
    }
    ...
    async function playBrowser(kind, cfg) {
      ...
      for (const item of pattern) {
        ...
        gain.gain.exponentialRampToValueAtTime(0.32 * v, t + 0.012)
    ...
    const SOUND_OPTIONS = [
      ...
      { value: 'complete', label: T('完成音 · 上行叮咚', 'Complete · ascending ding-dong') },
      { value: 'subcomplete', label: T('子任务音 · 单声叮', 'Subtask · single ding') },
      { value: 'approval', label: T('授权音 · 双音门铃', 'Approval · two-tone bell') },
      { value: 'question', label: T('提问音 · 三连上行', 'Question · three ascending notes') },
      { value: 'planreview', label: T('评审音 · 三连下行', 'Review · three descending notes') },
      { value: 'goalblocked', label: T('受阻音 · 低音三连', 'Blocked · low triad') },
      { value: 'interrupt', label: T('打断音 · 快下行双响', 'Interrupt · quick descending pair') },
      { value: 'jobfail', label: T('失败音 · 快三连下行', 'Failure · quick descending triad') },
      { value: 'jobdone', label: T('后台完成音 · 上行双音', 'Job done · ascending pair') },
